...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--yolo_model', type=str, dest='model_path', default = 'keras_yolo3/logs/0001/trained_weights_final.h5',
    ...
    parser.add_argument(# good default choices: inception_logo_features_200_trunc2, vgg16_logo_features_128
        '--features', type=str, dest='features', default = 'NASNet_logo_features_224.hdf5',
    ...
    if FLAGS.image:
        ...
        print("model_name = {} , flavor = {}".format(model_name,flavor))
        ## load pre-processed LITW features database
        features, brand_map, input_shape = load_features(FLAGS.features)
        # print("features = {} , brand_map = {} , input_shape = {}".format(features,brand_map,input_shape))
        ## load inception model
        # model_name = ""
        model, preprocess_input, input_shape = load_extractor_model(model_name, flavor)
        print("Load model {} , preprocess_input = {}, input_shape = {}".format(model,preprocess_input,input_shape))
        my_preprocess = lambda x: preprocess_input(utils.pad_image(x, input_shape))
        # print("my_preprocess = {}".format(my_preprocess))
